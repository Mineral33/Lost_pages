...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed("left"):
		...
		$ruka.scale.x = abs($ruka.scale.x)*-1
		$other_hand.scale.x = abs($other_hand.scale.x)*-1
	...
	if Input.is_action_pressed("right"):
		...
		$ruka.scale.x = abs($ruka.scale.x)
		$other_hand.scale.x = abs($other_hand.scale.x)
...
func stab_action():
	$IsFightingTimer.start()
	var overlapping_objects = $ruka/stabArea.get_overlapping_areas()
	invisibility_deactivate()
	if in_air_passive:
		check_in_air_passive()
	for area in overlapping_objects:
