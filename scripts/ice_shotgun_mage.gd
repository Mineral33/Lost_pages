...
var bomb = preload("res://scenes/projectiles/ice_projectile.tscn")
func _on_timer_timeout() -> void:
	if player:
		for i in range(randi_range(7,11)):
			...
			bomb_inst.fire((direction + spread).normalized(), randf_range(400,450))
